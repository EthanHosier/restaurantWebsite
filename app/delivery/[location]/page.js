    ...
    return (
        <>
            ...
            <section className='parallax flex-col'>
                ...
                <h1 className='text-tsecondary text-center mb-24'>Delivery - {toTitleCase(location)}</h1>
                <SmoothScrollButton target={"#order-options"} title={"ORDER NOW"}/>
            </section>
            ...
            <section className='normal text-center flex bg-primary' id='order-options'>
                <div className='max-w-6xl flex flex-col sm:flex-row gap-y-16'>
                    {DATA.deliveryOptions[location].map((d, i) => (
                        <div className='flex flex-col items-center justify-center flex-1'>
                            ...
                            <Link href={d.url}>
                                <p className="bg-accent text-tsecondary p-3 rounded md:-mt-4 text-xl font-semibold">Order via {d.name}</p>
                            </Link>
                        </div>
                    ))}
                </div>
